...
def cleaning_schedule(today=datetime.now()):
    try:
        ...
        from_server_address = settings.R_SERVER_HOST
        sources_url = '/ocpu/user/dgadmin/library/rsolar/R/cleaning_schedule_rf/json'
...
def save_cleaning_schedule(plant_name, stream_name, model_name, timestamp, actual, predicted, residual, residual_sum, losses, check_and_update=False):
    #ts           = timezone.now()
    #plantname    = 'palladam'
    #stream_name  = 'energy'
    #model_name   = 'svm'
    ...
    try:
        if check_and_update==True:
            try:
                ...
                predict.save()
                print("new value created: "+str(predict))
            except Exception as ex:
                print("Exception in creating new predict record: "+str(ex))
        else:
            print("save_cleaning_schedule() check_and_update False")
            predict = PredictedValues.objects.create(
                timestamp_type=settings.TIMESTAMP_TYPES.BASED_ON_TIMESTAMP_IN_DATA,
                count_time_period=settings.DATA_COUNT_PERIODS.DAILY,
                identifier=plant_name,
