...
# store action and config status by source
class ActionConfigStatusBySource(Model):
    source_key = columns.Text(partition_key=True, primary_key=True)
    update_time = columns.DateTime()
    action = columns.Integer(default=0)
    config = columns.Integer(default=0)


# store action parameters by stream
class ActionStorageByStream(Model):
    source_key = columns.Text(partition_key=True, primary_key=True)
    ...
    acknowledgement = columns.Integer(default=0)
    comments = columns.Text()


class Field_Action(models.Model):
    # associated sensor
