...
from .serializers import SourceActionValues,StreamActionValues,ActionFieldSerializer
...
class ActionStreamsViewSet(viewsets.ViewSet):
    """
    Manage your action streams
    """
    authentication_classes = (authentication.TokenAuthentication, authentication.SessionAuthentication,)
    permission_classes = (permissions.IsAuthenticated,)
    lookup_field = 'action_stream_name'

    def list(self, request, source_key=None):
        """
            Get a list of action streams for a source.
            ---
            response_serializer: ActionFieldSerializer
            responseMessages:
                - code : 200
                  message : Success
                - code : 400
        ...
        try:
            try:
                ...
                return log_and_return_error(request.user.id, request, request_arrival_time,
                                            ...
                                            False, comments)
            action_streams = ActionField.objects.filter(source=source)
            serializer = ActionFieldSerializer(action_streams, many=True)
    ...
    def retrieve(self, request, action_stream_name=None, source_key=None):
        """
            Get an action stream with the mentioned key.
            ---
            response_serializer: ActionFieldSerializer
            responseMessages:
                - code : 200
                ...
                - code : 400
                  message : If the source key mentioned is invalid (INVALID_SOURCE_KEY) or
                            If the specified action stream does not exist (INVALID_ACTION_DATA_STREAM).
                - code : 401
                  message : Not authenticated
        """
        request_arrival_time = timezone.now()
        try:
            ...
            return log_and_return_error(request.user.id, request, request_arrival_time,
                                        ...
                                        False, comments)
        try:
            action_stream = ActionField.objects.get(source=source,
                                                    name=action_stream_name)
            serializer = ActionFieldSerializer(action_stream,
                                          many=False)
            response = Response(serializer.data)
            log_a_success(request.user.id, request, response.status_code,
                          request_arrival_time, source_key=source_key)
            ...
            comments = generate_exception_comments(sys._getframe().f_code.co_name)
            return log_and_return_error(request.user.id, request, request_arrival_time,
                                        settings.ERRORS.INVALID_ACTION_DATA_STREAM, settings.RESPONSE_TYPES.DRF,
                                        False, comments)
        # check for broad errors (ones we don't know yet)
        except Exception as exception:
            comments = generate_exception_comments(sys._getframe().f_code.co_name)
    ...
    def create(self, request, source_key=None):
        """
            Create a new action data stream.
            ---
            parameters:
                - name: body
                  type: WriteActionFieldSerializer
            ...
            responseMessages:
                ...
                - code : 409
                  message : If a stream with the mentioned name OR position already exists (DUPLICATE_ACTION_STREAM).
                - code : 401
                  message : Not authenticated
        """
        request_arrival_time = timezone.now()
        ...
        # save the new stream
        try:
            serializer = ActionFieldSerializer(data=request.data)
            ...
            try:
                field = ActionField.objects.get(source=source,
            ...
            comments = generate_exception_comments(sys._getframe().f_code.co_name)
            return log_and_return_error(request.user.id, request, request_arrival_time,
                                        settings.ERRORS.DUPLICATE_ACTION_STREAM, settings.RESPONSE_TYPES.DRF,
                                        False, comments,
    ...
    def destroy(self, request, action_stream_name=None, source_key=None):
        """
            Delete an action data stream.
            ---
            responseMessages:
                - code : 204
